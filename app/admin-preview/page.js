...
export default function AdminPage() {
  ...
  // ════════════════════════════════════════════════════════════════
  // PREVIEW EXPERIMENTAL (/admin-preview)
  // Clone visual do /admin com DADOS DEMO. SEM auth, SEM Supabase, SEM
  // impacto em produção. Usuário fictício NÃO-owner (esconde features beta).
  // Para virar produção real, restaurar checkAndLoad/loadAll do app/admin.
  // ════════════════════════════════════════════════════════════════
  async function checkAndLoad() {
    setUser({ id: 'preview-user', email: 'preview@nexcontrol.app' })
    setProfile({ id: 'preview-user', role: 'admin', tenant_id: 'preview-tenant', nome: 'Bruno Oliveira' })
    loadAll()
  }

  async function loadAll() {
    const activeMetas = DEMO_METAS.filter(m => !m.deleted_at)
    const activeMetaIds = new Set(activeMetas.map(m => m.id))
    setOperators(DEMO_OPERATORS)
    setMetas(activeMetas)
    setTrashMetas(DEMO_METAS.filter(m => !!m.deleted_at))
    setRemessas(DEMO_REMESSAS.filter(r => activeMetaIds.has(r.meta_id)))
    setInvites([])
    setTenant({ id: 'preview-tenant', name: 'Preview', subscription_status: 'active' })
    setSub({ status: 'active' })
    setCosts(DEMO_COSTS || [])
    setMyMetas([]); setMyRem([])
    prevRemCount.current = DEMO_REMESSAS.length
    setLoading(false)
  }
      ...
      <div style={{ maxWidth:1380, margin:'0 auto', padding:'32px 28px' }}>
        ...
        {tab==='redes' && (
          <motion.div key="redes"
            ...
            transition={{ duration: 0.3, ease }}>
            <div style={{ display:'flex', alignItems:'center', gap:16, marginBottom:28 }}>
              <div style={{ width:44, height:44, borderRadius:12, background:'linear-gradient(135deg,rgba(209,250,229,0.15),rgba(255,255,255,0.1))', border:'1px solid var(--profit-border)', display:'flex', alignItems:'center', justifyContent:'center' }}>
