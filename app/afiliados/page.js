    ...
    <AppLayout userName={profile?.nome} userEmail={user?.email} isAdmin={true} tenant={tenant} userId={user?.id} tenantId={profile?.tenant_id}>
      <main style={{ minHeight: '100vh', padding: '40px 28px 80px', maxWidth: 1200, margin: '0 auto' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease }} style={{ marginBottom: 28, display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ width: 44, height: 44, borderRadius: 12, background: 'rgba(168,85,247,0.12)', border: '1px solid rgba(168,85,247,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width={22} height={22} viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="1.7" strokeLinecap="round"><circle cx="9" cy="7" r="4"/><path d="M3 21v-2a4 4 0 014-4h4a4 4 0 014 4v2"/><path d="M16 3.13a4 4 0 010 7.75"/><path d="M21 21v-2a4 4 0 00-3-3.87"/></svg>
          </div>
          <div>
        ...
        {!enabled ? (
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease }}
            style={{ borderRadius: 20, background: 'linear-gradient(145deg, #0c1424, #080e1a)', border: '1px solid rgba(168,85,247,0.15)', padding: '48px 36px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: '-30%', left: '20%', right: '20%', height: 200, background: 'radial-gradient(ellipse, rgba(168,85,247,0.12), transparent 70%)', filter: 'blur(50px)', pointerEvents: 'none' }} />
            <div style={{ position: 'relative', zIndex: 1 }}>
              <motion.div animate={{ scale: [1, 1.04, 1] }} transition={{ duration: 2.5, repeat: Infinity, ease: 'easeInOut' }}
                style={{ width: 72, height: 72, borderRadius: 20, background: 'rgba(168,85,247,0.08)', border: '1px solid rgba(168,85,247,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 22px' }}>
                <svg width={34} height={34} viewBox="0 0 24 24" fill="none" stroke="#a855f7" strokeWidth="1.6" strokeLinecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
              </motion.div>
              ...
              <div style={{ marginTop: 22, display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderRadius: 10, background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.14)' }}>
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, ease, delay: 0.05 }}
              style={{ borderRadius: 18, padding: 24, marginBottom: 22, background: 'linear-gradient(145deg, rgba(168,85,247,0.08), rgba(34,197,94,0.04))', border: '1px solid rgba(168,85,247,0.18)', position: 'relative', overflow: 'hidden' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
                <div>
                  <p style={{ fontSize: 10, fontWeight: 700, color: '#a855f7', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '0 0 4px' }}>Seu link de convite</p>
                  <p style={{ fontSize: 12, color: 'var(--t3)', margin: 0 }}>Compartilhe e ganhe <strong style={{ color: '#22C55E' }}>{Math.round((data?.rate || 0.30) * 100)}%</strong> de comissão recorrente</p>
                </div>
                <span style={{ fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 6, background: 'rgba(34,197,94,0.1)', color: '#22C55E', border: '1px solid rgba(34,197,94,0.2)' }}>ATIVO</span>
              </div>
              <div style={{ display: 'flex', gap: 10, alignItems: 'stretch', flexWrap: 'wrap' }}>
                ...
                <motion.button onClick={copyLink} whileTap={{ scale: 0.97 }}
                  style={{ padding: '12px 22px', borderRadius: 10, border: 'none', cursor: 'pointer', fontSize: 13, fontWeight: 700, background: copied ? 'rgba(34,197,94,0.18)' : '#a855f7', color: copied ? '#22C55E' : '#fff', display: 'inline-flex', alignItems: 'center', gap: 8, transition: 'background 0.2s' }}>
            ...
            <div className="g-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 22 }}>
              {[
                { l: 'Indicados', v: totals.totalIndicados || 0, c: '#a855f7', prefix: '' },
                { l: 'Faturamento gerado', v: totals.totalFaturado || 0, c: '#60A5FA', prefix: 'R$ ' },
                { l: 'Comissão acumulada', v: totals.totalComissao || 0, c: '#22C55E', prefix: 'R$ ' },
                { l: 'A receber', v: totals.pendente || 0, c: '#F59E0B', prefix: 'R$ ' },
              ].map((k, i) => (
                  ...
                  <AnimatePresence initial={false}>
                    {referrals.map(r => (
                      <motion.div key={r.tenant_id} layout
                        ...
                        <div style={{ width: 30, height: 30, borderRadius: 8, background: 'rgba(168,85,247,0.12)', border: '1px solid rgba(168,85,247,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <p style={{ fontSize: 10, color: 'var(--t4)', margin: '2px 0 0' }}>
                            ...
                            <span style={{ color: r.subscription_status === 'active' ? '#22C55E' : r.subscription_status === 'trial' ? '#F59E0B' : 'var(--t4)', fontWeight: 600 }}>
                        ...
                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                          <p style={{ fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 800, color: '#22C55E', margin: 0 }}>+R$ {fmt(r.commission)}</p>
