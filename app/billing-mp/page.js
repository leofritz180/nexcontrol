  ...
  return (
    <div style={cardStyleV2}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 22 }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 900, color: 'var(--t1)', margin: '0 0 6px', letterSpacing: '-0.025em' }}>
          Adicionar {added} operador{added !== 1 ? 'es' : ''}
        </h1>
        <p style={{ fontSize: 12.5, color: 'var(--t3)', margin: 0, lineHeight: 1.5 }}>
          Você tem <strong style={{ color: 'var(--t2)' }}>{opQty}</strong> operador{opQty !== 1 ? 'es' : ''} e seu plano cobre <strong style={{ color: 'var(--t2)' }}>{currentPaidOps}</strong>.<br/>
      ...
      <div style={{ padding: '14px 18px', borderRadius: 12, background: 'rgba(209,250,229,0.04)', border: '1px solid rgba(209,250,229,0.12)', marginBottom: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--t1)' }}>{fmtDate(currentExpires)}</span>
        </div>
  ...
  return (
    <div style={v2 ? cardStyleV2 : cardStyle}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 22 }}>
        ...
        <h1 style={{ fontSize: 24, fontWeight: 900, color: 'var(--t1)', margin: '0 0 4px', letterSpacing: '-0.025em' }}>
          {isRenewal ? 'Renove seu plano' : 'Escolha o periodo'}
        </h1>
        <p style={{ fontSize: 12.5, color: 'var(--t3)', margin: 0 }}>
          {planLabel} · <strong style={{ color: 'var(--t2)' }}>R$ {fmt(monthlyTier)}/mês</strong>
        </p>
      </div>
      ...
      {setOpQty && (
        <div style={{ marginBottom: 18, padding: '14px 16px', borderRadius: 14, background: 'var(--fill-1)', border: '1px solid var(--b1)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            <div>
              <div style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--t1)' }}>Operadores no plano</div>
              <div style={{ fontSize: 11, color: 'var(--t3)', marginTop: 2 }}>Você tem <strong style={{ color: 'var(--t2)' }}>{realOps}</strong> operador{realOps !== 1 ? 'es' : ''} ativo{realOps !== 1 ? 's' : ''}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <button type="button" aria-label="Remover" disabled={!canDec}
                onClick={() => canDec && setOpQty(q => Math.max(0, q - 1))}
                style={{ width: 34, height: 34, borderRadius: 9, border: '1px solid var(--b2)', background: canDec ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.02)', color: canDec ? '#fff' : 'var(--t4)', fontSize: 20, fontWeight: 700, cursor: canDec ? 'pointer' : 'not-allowed', lineHeight: 1 }}>−</button>
  ...
  return (
    <div style={cardStyle}>
      <div style={{ textAlign: 'center', marginBottom: 20 }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 14px', borderRadius: 999, background: 'var(--fill-2)', border: '1px solid var(--b3)', marginBottom: 14 }}>
          <motion.div
            animate={{ boxShadow: ['0 0 0 0 rgba(255,255,255,0.5)', '0 0 0 6px rgba(255,255,255,0)', '0 0 0 0 rgba(255,255,255,0)'] }}
            transition={{ duration: 1.8, repeat: Infinity, ease: 'easeInOut' }}
            style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--t1)' }}
          />
          <span style={{ fontSize: 10, color: 'var(--warn)', letterSpacing: '0.1em', fontWeight: 700, textTransform: 'uppercase' }}>Aguardando pagamento</span>
        </div>
        <h2 style={{ fontSize: 20, fontWeight: 800, color: 'var(--t1)', margin: '0 0 4px' }}>Escaneie o QR Code</h2>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '10px 14px', borderRadius: 10, background: 'var(--fill-2)', border: '1px solid var(--b2)' }}>
        <motion.div
          style={{ width: 12, height: 12, borderRadius: '50%', border: '2px solid var(--b3)', borderTopColor: 'rgba(255,255,255,0.78)' }}
          animate={{ rotate: 360 }}
          transition={{ duration: 1, repeat: Infinity, ease: 'linear' }}
        />
        <span style={{ fontSize: 11, color: 'var(--t2)', fontWeight: 600, letterSpacing: '0.04em' }}>
...
function ApprovedCard() {
  return (
    <div style={{ ...cardStyle, textAlign: 'center' }}>
      ...
      <h2 style={{ fontSize: 22, fontWeight: 800, color: 'var(--t1)', margin: '0 0 8px' }}>Pagamento aprovado!</h2>
      <p style={{ fontSize: 13, color: 'var(--t3)', margin: '0 0 16px' }}>Seu plano PRO foi ativado. Redirecionando...</p>
      <div style={{ width: '100%', height: 2, background: 'var(--fill-2)', borderRadius: 2, overflow: 'hidden' }}>
        <motion.div
  ...
  return (
    <div style={cardStyle}>
      <div style={{ textAlign: 'center', marginBottom: 18 }}>
        ...
        <h2 style={{ fontSize: 18, fontWeight: 800, color: 'var(--t1)', margin: '0 0 6px' }}>Nao foi possivel gerar a cobranca</h2>
        <p style={{ fontSize: 12, color: 'var(--loss)', margin: 0 }}>{error}</p>
      </div>
      <button
        onClick={onRetry}
        style={{
