      ...
      <div style={{maxWidth:820,margin:'0 auto',padding:'40px 28px'}}>
        ...
        <motion.div
          ...
          <motion.div
            ...
            style={{
              ...
              background: billing?.subActive
                ? 'linear-gradient(135deg, rgba(0,140,94,0.15), rgba(0,140,94,0.06))'
                : billing?.trialActive
              ...
              border:`1px solid ${billing?.subActive?'rgba(0,140,94,0.3)':billing?.trialActive?'rgba(255,255,255,0.3)':'rgba(239,68,68,0.3)'}`,
              boxShadow:`0 0 20px ${billing?.subActive?'rgba(0,140,94,0.15)':billing?.trialActive?'rgba(255,255,255,0.15)':'rgba(239,68,68,0.15)'}`,
            }}>
            <motion.div
              animate={{ boxShadow:[`0 0 0 0 ${billing?.subActive?'rgba(0,140,94,0.6)':billing?.trialActive?'rgba(255,255,255,0.6)':'rgba(239,68,68,0.6)'}`,`0 0 0 5px ${billing?.subActive?'rgba(0,140,94,0)':billing?.trialActive?'rgba(255,255,255,0)':'rgba(239,68,68,0)'}`,`0 0 0 0 rgba(0,0,0,0)`] }}
              transition={{ duration:2, repeat:Infinity, ease:'easeInOut' }}
              style={{ width:7, height:7, borderRadius:'50%', background:billing?.subActive?'#008C5E':billing?.trialActive?'rgba(255,255,255,0.78)':'#EF4444' }}
            />
            <span style={{ fontSize:11, fontWeight:800, color:billing?.subActive?'#008C5E':billing?.trialActive?'rgba(255,255,255,0.78)':'#EF4444', letterSpacing:'0.08em' }}>
          ...
          <motion.h1
            ...
            <span style={{ background:'linear-gradient(135deg, rgba(255,255,255,0.78), #008C5E)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>com estrutura profissional</span>
          </motion.h1>
          ...
          <motion.div
            ...
            {[
              { icon:<svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="#008C5E" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>, t:'Cancele quando quiser' },
              { icon:<svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="#008C5E" strokeWidth="2.5" strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>, t:'Pagamento 100% seguro' },
              { icon:<svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="#008C5E" strokeWidth="2.5" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>, t:'Liberacao imediata' },
            ].map((x,i)=>(
        ...
        <div className="a2 g-side" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:18,marginBottom:28}}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:9}}>
                {['Acesso completo ao painel','Gestao de metas e remessas','Faturamento e relatorios','Chaves PIX','Sem operadores'].map((t,i)=>(
                  <div key={i} style={{display:'flex',alignItems:'center',gap:8}}>
                    <div style={{ width:16, height:16, borderRadius:4, background: i<4?'rgba(0,140,94,0.12)':'rgba(148,163,184,0.08)', border:`1px solid ${i<4?'rgba(0,140,94,0.25)':'rgba(148,163,184,0.18)'}`, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                      <svg width={10} height={10} viewBox="0 0 24 24" fill="none" stroke={i<4?'#008C5E':'#64748B'} strokeWidth="3" strokeLinecap="round">
          ...
          <motion.div
            ...
            <div style={{padding:'26px 24px'}}>
              ...
              <div style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'3px 10px', borderRadius:6, background:'rgba(0,140,94,0.1)', border:'1px solid rgba(0,140,94,0.22)', marginBottom:16 }}>
                <svg width={10} height={10} viewBox="0 0 24 24" fill="none" stroke="#008C5E" strokeWidth="2.5" strokeLinecap="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>
                <span style={{ fontSize:10, color:'#008C5E', fontWeight:800, letterSpacing:'0.04em' }}>ATE 25% DE DESCONTO</span>
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:9}}>
                {[
                  ...
                  <div key={i} style={{display:'flex',alignItems:'center',gap:8}}>
                    <div style={{ width:16, height:16, borderRadius:4, background:'rgba(0,140,94,0.14)', border:'1px solid rgba(0,140,94,0.3)', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow: item.strong?'0 0 8px rgba(0,140,94,0.25)':'none' }}>
                      <svg width={10} height={10} viewBox="0 0 24 24" fill="none" stroke="#008C5E" strokeWidth="3" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
                    </div>
        ...
        {opQty > 0 && (
          <motion.div
            ...
            style={{
              ...
              backdropFilter:'blur(18px) saturate(150%)', WebkitBackdropFilter:'blur(18px) saturate(150%)',
              border:'1px solid rgba(0,140,94,0.14)',
              boxShadow:'0 8px 28px rgba(0,0,0,0.4), 0 0 32px rgba(0,140,94,0.04), inset 0 1px 0 rgba(255,255,255,0.04)',
            }}>
            <div style={{ position:'absolute', top:0, left:'15%', right:'15%', height:1, background:'linear-gradient(90deg, transparent, rgba(0,140,94,0.4), transparent)' }}/>

            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between', marginBottom:14, flexWrap:'wrap', gap:10}}>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                <div style={{
                  width:30, height:30, borderRadius:9,
                  background:'rgba(0,140,94,0.12)', border:'1px solid rgba(0,140,94,0.3)',
                  display:'flex', alignItems:'center', justifyContent:'center',
                  boxShadow:'0 0 12px rgba(0,140,94,0.18)',
                }}>
                  <svg width={13} height={13} viewBox="0 0 24 24" fill="none" stroke="#008C5E" strokeWidth="2.2" strokeLinecap="round"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17"/><polyline points="16 7 22 7 22 13"/></svg>
                </div>
                <div>
                ...
                return (
                  <motion.div key={i}
                    ...
                    style={{
                      ...
                      background: isActive
                        ? 'linear-gradient(145deg, rgba(0,140,94,0.15), rgba(0,140,94,0.03))'
                        : t.discount > 0 ? 'rgba(255,255,255,0.03)' : 'rgba(148,163,184,0.04)',
                      border:`${isActive?'1.5px':'1px'} solid ${isActive?'rgba(0,140,94,0.4)':'rgba(255,255,255,0.06)'}`,
                      boxShadow: isActive ? '0 6px 20px rgba(0,140,94,0.2), 0 0 24px rgba(0,140,94,0.1)' : 'none',
                      transition:'all 0.25s',
                    }}>
                    {isActive && (
                      <div style={{ position:'absolute', top:-4, left:'50%', transform:'translateX(-50%)', padding:'2px 8px', borderRadius:'0 0 6px 6px', background:'#008C5E', fontSize:7, fontWeight:900, color:'white', letterSpacing:'0.08em' }}>
                        ATIVO
                      </div>
                    )}
                    <p style={{fontSize:9, fontWeight:800, color:isActive?'#008C5E':'var(--t4)', letterSpacing:'0.08em', marginBottom:5, marginTop: isActive?4:0, textTransform:'uppercase'}}>{t.label}</p>
                    <p className="t-num" style={{fontSize:isActive?20:16, fontWeight:900, color:isActive?'#008C5E':t.discount>0?'var(--t1)':'var(--t4)', marginBottom:3, letterSpacing:'-0.02em', textShadow: isActive?'0 0 14px rgba(0,140,94,0.3)':'none'}}>
        ...
        {opQty > 0 && (
          <div className="a3" style={{borderRadius:22,overflow:'hidden',background:'var(--surface)',border:'1px solid var(--brand-border)',boxShadow:'0 0 40px rgba(255,255,255,0.06)',marginBottom:24}}>
            ...
            <div style={{padding:'24px 28px'}}>
              {/* Discount badge */}
              {price.discount > 0 && (
                <div style={{display:'flex',alignItems:'center',gap:8,padding:'10px 14px',borderRadius:10,background:'rgba(0,140,94,0.06)',border:'1px solid rgba(0,140,94,0.12)',marginBottom:14}}>
        ...
        {!billing?.subActive && (
          <div className="a4" style={{marginBottom:32}}>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,marginTop:12}}>
              ...
              <span style={{fontSize:10,color:'rgba(0,140,94,0.5)',fontWeight:600}}>Pagamento seguro via Pix · Ativacao instantanea</span>
            </div>
          </div>
        )}
