        ...
        <form onSubmit={submit}>
          <div style={{ marginBottom: 12 }}>
            ...
            <select value={operatorId} onChange={e => setOperatorId(e.target.value)} style={{ ...inp, cursor: 'pointer' }}>
              <option value={leaderId} style={{ background: 'var(--surface)' }}>Eu (líder)</option>
              {operators.filter(o => o.id !== leaderId).map(o => (
                <option key={o.id} value={o.id} style={{ background: 'var(--surface)' }}>{o.nome || o.email?.split('@')[0]}</option>
              ))}
            </select>
          </div>
          <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
            ...
            <div>
              ...
              <select value={rede} onChange={e => setRede(e.target.value)} style={{ ...inp, cursor: 'pointer' }}>
                <option value="" style={{ background: 'var(--surface)' }}>Selecione</option>
                {REDES.map(r => <option key={r} value={r} style={{ background: 'var(--surface)' }}>{r}</option>)}
              </select>
            </div>
          </div>
          <div style={{ marginBottom: 12 }}>
...
function CostModal({ leaderId, onClose, onSaved }) {
  ...
  const inp = { width: '100%', padding: '10px 12px', borderRadius: 10, fontSize: 13, fontFamily: 'inherit', background: 'var(--fill-2)', border: '1px solid var(--b2)', color: 'var(--t1)' }
  const lbl = { display: 'block', fontSize: 11, fontWeight: 600, color: 'var(--t3)', margin: '0 0 6px' }
        ...
        <form onSubmit={submit}>
          <div style={{ marginBottom: 12 }}>
            ...
            <select value={type} onChange={e => setType(e.target.value)} style={{ ...inp, cursor: 'pointer' }}>
              {COST_TYPES.map(t => <option key={t.id} value={t.id} style={{ background: 'var(--surface)' }}>{t.label}</option>)}
            </select>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
            <div>
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--surface)' }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--t3)' }}>Carregando equipe...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <AppLayout userName={getName(profile)} userEmail={user?.email} isAdmin={false} userId={user?.id} tenantId={profile?.tenant_id}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 20px' }}>
        ...
        <motion.div {...fadeUp(0)} style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 12px', borderRadius: 20, background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.3)', marginBottom: 12 }}>
              <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', color: 'var(--loss)', textTransform: 'uppercase' }}>Painel do líder</span>
            </div>
        ...
        {denied && (
          <div style={{ padding: '14px 16px', borderRadius: 12, background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: 'var(--loss)', fontSize: 13, marginBottom: 20 }}>
        ...
        <motion.div {...fadeUp(2)} style={{ marginBottom: 32 }}>
          ...
          {ranking.length === 0 ? (
            <div style={{ padding: '36px 20px', textAlign: 'center', borderRadius: 14, background: 'var(--fill-1)', border: '1px solid var(--b1)' }}>
                  ...
                  <motion.div key={op.id} {...fadeUp(idx, 0.1)} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        ...
                        {op.is_team_leader && <span style={{ fontSize: 9, fontWeight: 800, color: 'var(--loss)', padding: '2px 7px', borderRadius: 5, background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)' }}>LÍDER</span>}
        ...
        <motion.div {...fadeUp(3)}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, gap: 10, flexWrap: 'wrap' }}>
            ...
            {toClose.length > 0 && (
              <span style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--warn)', padding: '4px 10px', borderRadius: 20, background: 'rgba(255,209,102,0.12)', border: '1px solid rgba(255,209,102,0.3)' }}>
        ...
        <motion.div {...fadeUp(4)} style={{ marginTop: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14, gap: 10, flexWrap: 'wrap' }}>
            ...
            <button type="button" onClick={() => setShowCost(true)}
              style={{ padding: '9px 16px', borderRadius: 10, border: '1px solid var(--b2)', cursor: 'pointer', fontFamily: 'inherit', background: 'var(--fill-2)', color: 'var(--t1)', fontSize: 12.5, fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: 7 }}>
              <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              Novo custo
            </button>
          </div>
          {costs.length === 0 ? (
            <div style={{ padding: '28px 20px', textAlign: 'center', borderRadius: 14, background: 'var(--fill-1)', border: '1px solid var(--b1)' }}>
              <p style={{ fontSize: 13, color: 'var(--t3)', margin: 0 }}>Nenhum custo lançado.</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {costs.map(c => (
                <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 12, background: 'var(--fill-1)', border: '1px solid var(--b1)' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <button type="button" onClick={() => deleteCost(c.id)} title="Remover"
                    style={{ width: 30, height: 30, borderRadius: 8, border: '1px solid var(--b1)', background: 'transparent', color: 'var(--t4)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
