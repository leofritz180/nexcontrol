...
export default function HomePage() {
  ...
  return (
    <main style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:32, textAlign:'center', position:'relative', zIndex:1 }}>
      {/* Orbs */}
      <div style={{ position:'fixed', top:'-20%', left:'-10%', width:600, height:600, borderRadius:'50%', background:'radial-gradient(circle, rgba(59,130,246,0.1), transparent 65%)', filter:'blur(60px)', pointerEvents:'none' }}/>
      <div style={{ position:'fixed', bottom:'-20%', right:'-10%', width:500, height:500, borderRadius:'50%', background:'radial-gradient(circle, rgba(34,197,94,0.07), transparent 65%)', filter:'blur(60px)', pointerEvents:'none' }}/>

      <div className="a1" style={{ maxWidth:560, position:'relative', zIndex:1 }}>
        {/* Logo */}
        <div style={{ width:64, height:64, borderRadius:18, background:'linear-gradient(135deg,#3B82F6,#3B82F6)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 28px', boxShadow:'0 0 60px rgba(59,130,246,0.4)', animation:'float 4s ease-in-out infinite' }}>
          <svg width={28} height={28} viewBox="0 0 28 28" fill="none">
            ...
            <circle cx="21" cy="8" r="3" fill="#22C55E"/>
          </svg>
        </div>
        ...
        <h1 style={{ fontFamily:'Inter,sans-serif', fontWeight:900, fontSize:44, letterSpacing:'-0.04em', color:'var(--t1)', margin:'0 0 12px', lineHeight:1.1 }}>
          Nex<span style={{ background:'linear-gradient(135deg,#60A5FA,#60A5FA)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>Control</span>
        </h1>
