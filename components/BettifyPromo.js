...
function GigaMeter() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '18px 18px 16px', borderRadius: 14, background: 'var(--fill-1)', border: '1px solid var(--b1)' }}>
      {/* outras lojas */}
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 7 }}>
          <span style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--t3)' }}>Outras lojas</span>
          <span style={{ fontSize: 11.5, fontWeight: 800, fontFamily: 'var(--mono, monospace)', color: 'var(--loss)' }}>paga 10GB → recebe 7GB</span>
        </div>
        <div style={{ position: 'relative', height: 12, borderRadius: 7, background: 'var(--fill-2)', overflow: 'hidden' }}>
        ...
        <p style={{ fontSize: 10.5, color: 'var(--t4)', margin: '5px 0 0' }}>3GB somem no caminho — a famosa burla de giga</p>
      </div>
      {/* bettify */}
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 7 }}>
        ...
        <p style={{ fontSize: 10.5, color: 'var(--t4)', margin: '5px 0 0' }}>giga contada certinha, em tempo real, dentro do painel</p>
      </div>
    </div>
  )
}
    ...
    <AnimatePresence>
      {want && granted && (
        <motion.div
          ...
          style={{ position: 'fixed', inset: 0, zIndex: 9000, background: 'rgba(17,19,24,0.55)', backdropFilter: 'blur(14px)', WebkitBackdropFilter: 'blur(14px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          onClick={fechar}
        >
          <motion.div
          ...
          >
            ...
            <button type="button" onClick={fechar} aria-label="Fechar"
              style={{ position: 'absolute', top: 14, right: 14, width: 32, height: 32, borderRadius: 9, background: 'var(--fill-2)', border: '1px solid var(--b1)', color: 'var(--t2)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--t1)', margin: 0 }}>Bettify Proxy · integrada ao seu painel</p>
              </div>
            </div>

            {/* headline */}
            <h2 style={{ fontSize: 27, fontWeight: 900, letterSpacing: '-0.03em', color: 'var(--t1)', margin: '0 0 8px', lineHeight: 1.12 }}>
            ...
            <p style={{ fontSize: 14, color: 'var(--t2)', margin: '0 0 18px', lineHeight: 1.6 }}>
              Proxies de qualidade feitas pra <strong style={{ color: 'var(--t1)' }}>operação de CPA</strong> — preço justo e a giga
              que você pagou chegando <strong style={{ color: 'var(--t1)' }}>inteira</strong>. Sem a “burla de giga” das lojas
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9, margin: '18px 0 22px' }}>
              {[
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--t1)', fontWeight: 500 }}>{t}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12 }}>
              <a href="https://bettifyproxy.com" target="_blank" rel="noopener noreferrer" onClick={marcarVisto}
                style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--t2)', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                bettifyproxy.com
              ...
              <button type="button" onClick={fechar} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 12.5, color: 'var(--t3)', fontFamily: 'inherit', padding: 6 }}>
                Agora não
              </button>
            </div>
          </motion.div>
