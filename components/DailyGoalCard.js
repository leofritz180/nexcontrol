    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', maxWidth: 340 }}>
      <div style={{ display: 'flex', gap: 8 }}>
        <input autoFocus value={v} onChange={e => setV(e.target.value.replace(/\D/g, '').slice(0, 7))}
          ...
          style={{ flex: 1, padding: '12px 14px', borderRadius: 11, background: 'rgba(0,0,0,0.32)', border: '1px solid var(--b3)', color: 'var(--t1)', fontSize: 16, fontFamily: 'var(--mono)', fontWeight: 800, outline: 'none' }} />
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        {[500, 1000, 2000, 5000].map(s => (
          <button key={s} onClick={() => setV(String(s))} style={{ padding: '5px 11px', borderRadius: 8, border: '1px solid var(--b3)', background: 'var(--fill-3)', color: 'var(--t1)', fontSize: 12, fontWeight: 800, cursor: 'pointer', fontFamily: 'var(--mono)' }}>{fmt(s)}</button>
        ))}
        {onCancel && <button onClick={onCancel} style={{ padding: '5px 11px', borderRadius: 8, border: 'none', background: 'none', color: 'var(--t2)', fontSize: 12, cursor: 'pointer' }}>cancelar</button>}
      </div>
    </div>
  )
}
...
export default function DailyGoalCard({ data, onSave }) {
  ...
  const iconTile = (IconCmp, sz = 52, isz = 24) => (
    <div style={{ width: sz, height: sz, borderRadius: 15, flexShrink: 0, background: 'linear-gradient(160deg, var(--fill-3), rgba(0,0,0,0.28))', border: `1px solid ${st.border}`, boxShadow: `inset 0 1px 0 rgba(255,255,255,0.1), 0 6px 18px rgba(0,0,0,0.35)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: st.accent }}>
      <IconCmp size={isz} />
    </div>
  )
    ...
    return (
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 18, flexWrap: 'wrap' }}>
          <div style={{ width: 58, height: 58, borderRadius: 16, background: 'linear-gradient(160deg, var(--fill-3), rgba(0,0,0,0.28))', border: `1px solid ${p.border}`, boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: p.accent, flexShrink: 0 }}>
            <TargetIco size={28} />
          </div>
          <div style={{ flex: 1, minWidth: 200 }}>
            <p style={{ margin: 0, fontSize: 18, fontWeight: 900, color: 'var(--t1)', letterSpacing: '-0.02em' }}>Defina sua meta de lucro do dia</p>
            <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--t2)', lineHeight: 1.5 }}>Qual o lucro que o time deve bater por dia? Acompanhe o progresso e mantenha a sequência.</p>
          </div>
          {editing
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 24, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                <span style={{ fontSize: 35, fontWeight: 900, color: 'var(--t1)', fontFamily: 'var(--mono)', letterSpacing: '-0.02em', lineHeight: 1, textShadow: '0 2px 12px rgba(0,0,0,0.45)' }}>{fmt(today)}</span>
                <span style={{ fontSize: 15, color: 'var(--t2)', fontFamily: 'var(--mono)', fontWeight: 700 }}>/ {fmt(target)}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 7, gap: 8 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--t1)', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                {hit
              ...
              {best > 0 && <span style={{ fontSize: 11.5, color: 'var(--t2)', flexShrink: 0, whiteSpace: 'nowrap', display: 'inline-flex', alignItems: 'center', gap: 5 }}><TrophyIco size={13} color="rgba(255,255,255,0.6)" /> recorde {fmt(best)}</span>}
            </div>
            <div style={{ position: 'relative', height: 11, borderRadius: 6, background: 'rgba(0,0,0,0.34)', overflow: 'hidden', border: '1px solid var(--b1)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            ...
            <button onClick={() => setEditing(true)} title="Alterar meta" style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(0,0,0,0.26)', border: '1px solid var(--b3)', color: 'var(--t1)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
