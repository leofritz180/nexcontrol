    ...
    <AnimatePresence>
      {show && granted && (
        <motion.div
          ...
          onClick={dismiss}
          style={{ position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(17,19,24,0.55)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 18 }}
        >
          <motion.div
          ...
          >
            ...
            <h2 style={{ margin: '0 0 8px', fontSize: 24, fontWeight: 900, color: 'var(--t1)', letterSpacing: '-0.03em' }}>O Network chegou 🎉</h2>
            <p style={{ margin: '0 0 16px', fontSize: 13.5, color: 'var(--t2)', lineHeight: 1.55 }}>
              A comunidade fechada dos <strong style={{ color: 'var(--t1)' }}>admins da NexControl</strong>. Troque experiência, dúvidas e oportunidades com quem também opera de verdade.
            </p>
            ...
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 14px', borderRadius: 10, background: 'rgba(245,180,60,0.1)', border: '1px solid rgba(245,180,60,0.3)', marginBottom: 22 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--warn)' }}>Os primeiros 100 a entrar ganham o selo <strong style={{ color: 'var(--t1)' }}>Veterano</strong></span>
            </div>
            ...
            <button type="button" onClick={dismiss} style={{ display: 'block', width: '100%', marginTop: 12, padding: 6, background: 'none', border: 'none', color: 'var(--t3)', fontSize: 12, cursor: 'pointer' }}>agora não</button>
          </motion.div>
        </motion.div>
      )}
    </AnimatePresence>
