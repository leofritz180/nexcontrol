...
const GROUP_ORDER = ['Operação', 'Financeiro', 'Recursos', 'Comunidade', 'Conta', 'Master']
...
const ADMIN_NAV = [
  ...
  { href:'/pix', label:'Chaves PIX', icon:'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z', group:'Financeiro' },
  { href:'/slots', label:'Slots Premium', icon:'M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z', pro: true, group:'Recursos' },
  { href:'/proxy', label:'Loja Proxy', icon:'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z', bettify: true, group:'Recursos' },
  { href:'/minhas-proxies', label:'Minhas Proxies', icon:'M2 20h.01M7 20v-4M12 20v-8M17 20V8M22 20V4', group:'Recursos' },
  { href:'/afiliados', label:'Afiliados', icon:'M17 20h5v-2a3 3 0 00-5.356-1.857M13 7a4 4 0 11-8 0 4 4 0 018 0zm6 3a2 2 0 100-4 2 2 0 000 4zM9 13a4 4 0 00-4 4v2h8v-2a4 4 0 00-4-4z', group:'Conta' },
  { href:'/billing', label:'Assinatura', icon:'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z', group:'Conta' },
  { href:'/tutorial', label:'Tutorial', icon:'M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664zM21 12a9 9 0 11-18 0 9 9 0 0118 0z', group:'Conta' },
]

const OP_NAV = [
  { href:'/operator', label:'Painel', icon:'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z', group:'Operação' },
  { href:'/performance', label:'Performance', icon:'M13 7h8m0 0v8m0-8l-8 8-4-4-6 6', group:'Operação' },
  { href:'/slots', label:'Slots Premium', icon:'M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a2 2 0 110 4v3a2 2 0 002 2h14a2 2 0 002-2v-3a2 2 0 110-4V7a2 2 0 00-2-2H5z', pro: true, group:'Recursos' },
  { href:'/proxy', label:'Loja Proxy', icon:'M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z', bettify: true, group:'Recursos' },
  { href:'/minhas-proxies', label:'Minhas Proxies', icon:'M2 20h.01M7 20v-4M12 20v-8M17 20V8M22 20V4', group:'Recursos' },
  { href:'/pix', label:'Chaves PIX', icon:'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z', group:'Conta' },
]
...
export default function Sidebar({ userName, userEmail, isAdmin, tenant, subscription, userId, tenantId }) {
  ...
  useEffect(() => {
    setNoir(temaEscuro())
    const aoTrocar = e => setNoir(!!e.detail?.escuro)
    window.addEventListener('nx:tema', aoTrocar)
    return () => window.removeEventListener('nx:tema', aoTrocar)
  }, [])
  ...
  // Agrupa por tema, na ordem de GROUP_ORDER (só grupos com item aparecem)
  const grouped = GROUP_ORDER
    .map(title => ({ title, items: allItems.filter(it => (it.group || 'Conta') === title) }))
  ...
  const content = (
    <div style={{ display:'flex', flexDirection:'column', height:'100%' }}>
          ...
          <span style={{ display:'flex', alignItems:'center', gap:10 }}>
            <span className="nx-logo-mark" style={{ position:'relative', display:'inline-flex', flexShrink:0 }}>
              ...
              <span aria-hidden style={{
                ...
                background:'linear-gradient(135deg, var(--k-laranja2), var(--k-laranja))',
              ...
              <span style={{ display:'flex', flexDirection:'column', lineHeight:1 }}>
                <span style={{ display:'flex', alignItems:'center', gap:8 }}>
                  ...
                  <span className="nx-selo-20" style={{
                    ...
                    background:'linear-gradient(135deg, var(--k-laranja2), var(--k-laranja))',
                    boxShadow:'0 4px 12px color-mix(in srgb, var(--k-laranja) 38%, transparent)',
                  }}>2.0</span>
                </span>
      ...
      <nav style={{ flex:1, padding:'0 12px', display:'flex', flexDirection:'column', gap:2, overflowY:'auto' }}>
        {grouped.map((grp, gi) => (
          <div key={grp.title} style={{ display:'flex', flexDirection:'column', gap:2, marginTop: gi === 0 ? 0 : 14 }}>
            {/* cabeçalho do tema (some quando o menu colapsa) */}
            <div className="sb-label" style={{ padding:'0 12px 4px', fontSize:9.5, fontWeight:800, letterSpacing:'0.16em', textTransform:'uppercase', color:'var(--t4)', fontFamily:'var(--mono, monospace)' }}>
              {grp.title}
            </div>
            {/* divisor curto (visível também no menu colapsado) */}
            <div className="sb-divider" style={{ display:'none', height:1, margin:'2px 12px 6px', background:'var(--b1)' }} />
            {grp.items.map(item => {
          const active = pathname === item.href || (item.href !== '/' && pathname.startsWith(item.href))
          const bettify = item.bettify
          const BETT_BG = 'linear-gradient(135deg, color-mix(in srgb, var(--k-laranja2) 20%, transparent), color-mix(in srgb, var(--k-laranja) 5%, transparent))'
          const BETT_BG_HOVER = 'linear-gradient(135deg, color-mix(in srgb, var(--k-laranja2) 32%, transparent), color-mix(in srgb, var(--k-laranja) 11%, transparent))'
          return (
            <Link key={item.href} href={item.href} onClick={()=>setMobileOpen(false)}
              ...
              style={{
                ...
                border: bettify ? '1px solid color-mix(in srgb, var(--k-laranja2) 42%, transparent)' : '1px solid transparent',
                boxShadow: bettify ? '0 0 18px color-mix(in srgb, var(--k-laranja) 14%, transparent)' : 'none',
                transition:'background 0.15s ease, color 0.15s ease, box-shadow 0.15s ease',
              }}
              onMouseEnter={e=>{ if(bettify){ e.currentTarget.style.background=BETT_BG_HOVER; e.currentTarget.style.boxShadow='0 6px 22px color-mix(in srgb, var(--k-laranja) 26%, transparent)' } else if(!active){e.currentTarget.style.background='var(--raised)';e.currentTarget.style.color='var(--t2)'} }}
              onMouseLeave={e=>{ if(bettify){ e.currentTarget.style.background=BETT_BG; e.currentTarget.style.boxShadow='0 0 18px color-mix(in srgb, var(--k-laranja) 14%, transparent)' } else if(!active){e.currentTarget.style.background='transparent';e.currentTarget.style.color='var(--t3)'} }}
            >
              {bettify ? (
                <img className="sb-ico" src="/bettify-logo.png" alt="Bettify" width={redesign?22:18} height={redesign?22:18}
                  style={{ flexShrink:0, objectFit:'contain', filter:'drop-shadow(0 0 7px color-mix(in srgb, var(--k-laranja2) 60%, transparent))' }} />
              ) : (
              ...
              {bettify && (
                <span className="sb-label" style={{
                  ...
                  background:'color-mix(in srgb, var(--k-laranja2) 18%, transparent)', color:'#ff9a78', border:'1px solid color-mix(in srgb, var(--k-laranja2) 40%, transparent)',
              ...
              {item.network && netUnread && (
                <span style={{
                  ...
                  background:'var(--k-laranja)', boxShadow:'0 0 9px color-mix(in srgb, var(--k-laranja) 85%, transparent)',
              ...
              {item.network && !netUnread && new Date() < new Date(NETWORK_NEW_UNTIL) && (
                <span className="sb-label" style={{
                  marginLeft:'auto', fontSize:8, fontWeight:800, padding:'2px 6px', borderRadius:4,
                  background:'linear-gradient(135deg, var(--k-laranja2), var(--k-laranja))', color:'#fff', border:'1px solid color-mix(in srgb, var(--k-laranja2) 45%, transparent)',
                  letterSpacing:'0.08em', boxShadow:'0 4px 14px color-mix(in srgb, var(--k-laranja) 45%, transparent)',
      ...
      {isAdmin && !subActive && (
        <div style={{ padding:'0 12px 12px' }}>
          <Link href="/billing" onClick={()=>setMobileOpen(false)}
            ...
            onMouseEnter={e=>{e.currentTarget.style.background='linear-gradient(135deg, var(--k-laranja2), var(--k-laranja))';e.currentTarget.style.color='#fff'}}
          ...
          {pushState === 'granted' ? (
            ...
            <button onClick={enablePush} disabled={pushBusy}
              style={{
                ...
                color:'#fff', background:'linear-gradient(135deg, var(--k-laranja2), var(--k-laranja))', border:'none',
              ...
              onMouseLeave={e=>{ if(!pushBusy) e.currentTarget.style.background='linear-gradient(135deg, var(--k-laranja2), var(--k-laranja))' }}
            >
        ...
        {nex2 && (
          <button type="button" onClick={alternarNoir} aria-pressed={noir}
            ...
            <span className="sb-label" aria-hidden style={{
              ...
              background: noir ? '#C8F21D' : 'var(--fill-3)', transition:'background .2s ease',
            }}>
              <span style={{
    ...
    <>
      ...
      <motion.button
        ...
        {!mobileOpen && userId && tenantId && (pushState === 'default' || pushState === 'prompt') && (
          <span style={{ position:'absolute', top:5, right:5, width:9, height:9, borderRadius:'50%', background:'var(--k-laranja)', border:'2px solid #000', animation:'notif-pulse 1.8s ease-in-out infinite' }}/>
        )}
      </motion.button>
      ...
      <style jsx global>{`
        @keyframes notif-pulse {
          0%, 100% { box-shadow: 0 0 0 0 color-mix(in srgb, var(--k-laranja) 50%, transparent); }
          60% { box-shadow: 0 0 0 7px color-mix(in srgb, var(--k-laranja) 0%, transparent); }
        }
        ...
        /* Colapsada: mostra o divisor curto no lugar do cabeçalho de tema */
        .sb-rd:not(:hover) .sb-divider { display: block !important; }
