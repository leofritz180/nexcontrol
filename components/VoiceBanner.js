    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          onClick={dismiss}
          style={{ position: 'fixed', inset: 0, zIndex: 3000, background: 'rgba(17,19,24,0.55)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
        >
          <motion.div
            initial={{ opacity: 0, y: 18, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 12, scale: 0.98 }}
            transition={{ duration: 0.3, ease: [0.33, 1, 0.68, 1] }}
            onClick={e => e.stopPropagation()}
            style={{ position: 'relative', width: '96%', maxWidth: 780, borderRadius: 18, overflow: 'hidden', background: 'var(--raised)', boxShadow: '0 30px 80px rgba(0,0,0,0.75), 0 0 50px rgba(225,0,0,0.12)' }}
          >
            ...
            <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 8, background: 'var(--surface)', borderTop: '1px solid var(--b1)' }}>
              ...
              <button type="button" onClick={dismiss}
                style={{ width: '100%', padding: '4px', background: 'transparent', border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, color: 'var(--t3)', fontFamily: 'inherit' }}
              >Agora não</button>
            </div>

            {/* Fechar */}
            <button type="button" onClick={dismiss} aria-label="Fechar"
              style={{ position: 'absolute', top: 12, right: 12, width: 32, height: 32, borderRadius: 9, background: 'rgba(0,0,0,0.5)', border: '1px solid var(--b3)', color: 'var(--t1)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0, backdropFilter: 'blur(4px)' }}
            >
