...
export default function VoiceCommandPanel({ userEmail }) {
  ...
  useEffect(() => {
    handleCommandRef.current = (heard) => {
      ...
      // 1) Acoes de Metodos (lucro 150 / novo metodo) — admins (financeiro)
      if (isAdminLevel) for (const a of ACTION_COMMANDS) {
        ...
        {open && (
          ...
          >
            ...
            {(transcript || interim || lastAction) && (
              <div style={{ padding: '10px 16px', borderBottom: '1px solid var(--b1)', background: 'var(--fill-1)' }}>
                {interim && (
                ...
                {transcript && (
                  <div style={{ fontSize: 11, color: 'var(--t1)', fontWeight: 600, marginBottom: lastAction ? 4 : 0 }}>
