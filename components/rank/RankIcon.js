...
export default function RankIcon({ name, size = 20, color = 'currentColor' }) {
  // No bento claro o icone fica branco sobre a pastilha colorida: traco 1.6 e
  // preenchimento de 15% somem nesse contraste, entao ambos sobem um pouco.
  const claro = useBento()
  const fo = (v) => claro ? Math.min(0.58, v + 0.16) : v
  const props = { width: size, height: size, viewBox: '0 0 24 24', fill: 'none', stroke: color, strokeWidth: claro ? 2 : 1.6, strokeLinecap: 'round', strokeLinejoin: 'round' }
  switch (name) {
    case 'shield': return (
      <svg {...props}><path d="M12 2L4 5v7c0 5 3.5 9 8 10 4.5-1 8-5 8-10V5l-8-3z" /></svg>
    )
    case 'shield-star': return (
      <svg {...props}><path d="M12 2L4 5v7c0 5 3.5 9 8 10 4.5-1 8-5 8-10V5l-8-3z" /><path d="M12 8l1.2 2.4 2.6.4-1.9 1.8.5 2.6L12 14l-2.4 1.2.5-2.6L8.2 10.8l2.6-.4z" fill={color} fillOpacity={fo(0.3)}/></svg>
    )
    ...
    case 'crown-shield': return (
      <svg {...props}><path d="M12 2L4 5v7c0 5 3.5 9 8 10 4.5-1 8-5 8-10V5l-8-3z" /><path d="M8 10l1.5 3 2.5-2 2.5 2 1.5-3v3H8z" fill={color} fillOpacity={fo(0.4)}/></svg>
    )
    case 'crystal': return (
    ...
    case 'lightning': return (
      <svg {...props}><path d="M13 2L4 14h7l-1 8 9-12h-7z" fill={color} fillOpacity={fo(0.2)}/></svg>
    )
    case 'flame': return (
      <svg {...props}><path d="M12 22c4 0 7-3 7-7 0-3-2-5-3-7-1 2-2 3-3 3-1-3-1-5-3-9-3 4-5 8-5 13 0 4 3 7 7 7z" fill={color} fillOpacity={fo(0.15)}/></svg>
    )
    case 'eye': return (
    ...
    case 'crown-stars': return (
      <svg {...props}><path d="M3 9l3 9h12l3-9-5 3-4-6-4 6z" fill={color} fillOpacity={fo(0.2)}/><circle cx="12" cy="3" r="1" fill={color}/><circle cx="6" cy="6" r="0.7" fill={color}/><circle cx="18" cy="6" r="0.7" fill={color}/><circle cx="2" cy="11" r="0.5" fill={color}/><circle cx="22" cy="11" r="0.5" fill={color}/></svg>
    )
    case 'apex': return (
      <svg {...props}><circle cx="12" cy="12" r="9" stroke={color} fill={color} fillOpacity={fo(0.05)}/><path d="M12 3l2 6 6 1-4.5 4.5L17 21l-5-3-5 3 1.5-6.5L4 10l6-1z" fill={color} fillOpacity={fo(0.4)}/><circle cx="12" cy="12" r="2" fill={color}/></svg>
    )
    default: return (
