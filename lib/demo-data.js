...
/* ═══════════════════════════════════
   HELPER
   ═══════════════════════════════════ */
export function shouldShowDemo(metas, userId) {
  // Demo mode foi DESATIVADO em prol do Product Tour (v53+).
  // Admin sem meta agora ve dashboard real zerado + tour automatico
  // (componente ProductTour disparado no /admin).
  // Mantemos a funcao exportada pra nao quebrar imports antigos.
  return false
}

export function exitDemoMode(userId) {
  if (!userId || typeof window === 'undefined') return
  try { localStorage.setItem(`nx_demo_exited_${userId}`, '1') } catch {}
}

export function isDemoExited(userId) {
  if (!userId || typeof window === 'undefined') return false
  try { return localStorage.getItem(`nx_demo_exited_${userId}`) === '1' } catch { return false }
}
