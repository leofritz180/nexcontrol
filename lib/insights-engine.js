...
// Entrega pelo catálogo (tipo alerta-operacao) via lib/pushClient.dispararPush
// ═══════════════════════════════════════
...
async function sendInsight(userId, dados) {
  try {
    // sai pelo catálogo como 'alerta-operacao' (fica na tela, botões
    // "Abrir meta" e "Ver slots"); o texto é escrito lá, na voz da pessoa
    const { dispararPush } = await import('./pushClient')
    await dispararPush('alerta-operacao', { ...dados, chave: dados.metaId ? String(dados.metaId) : undefined }, 'eu')
  } catch {
...
function checkSequenciaNegativa(remessas, meta, userId) {
  ...
  return { insight: 'sequencia_negativa', streak, total: streakTotal, metaId: meta.id, url: `/meta/${meta.id}` }
}
...
function checkPrejuizoAcimaMedia(remessas, novaRemessa, meta, userId) {
  ...
  return { insight: 'prejuizo_acima_media', perConta: prejPerConta, media: avgPrej, metaId: meta.id, url: `/meta/${meta.id}` }
}
...
function checkMetaParada(remessas, meta, userId, horasLimite = 12) {
  ...
  if (!remessas || remessas.length === 0) {
    ...
    return { insight: 'meta_sem_inicio', contas: meta.quantidade_contas || 0, rede: meta.rede, metaId: meta.id, url: `/meta/${meta.id}` }
  }
  ...
  markSent('meta_parada', meta.id, userId)
  return { insight: 'meta_parada', horas: Math.round(horas), metaId: meta.id, url: `/meta/${meta.id}` }
}
...
export async function evaluateOnLoad({ remessas, meta, userId }) {
  ...
  if (insight) {
    await sendInsight(userId, insight)
  }
}
