...
async function sendInsight(userId, title, body, url) {
  try {
    ...
    // "Abrir meta" e "Ver slots"); o metaId vem da própria url
    const metaId = (String(url || '').match(/\/meta\/([^/?]+)/) || [])[1]
    const { dispararPush } = await import('./pushClient')
    await dispararPush('alerta-operacao', { titulo: title, corpo: body, url, metaId, chave: metaId }, 'eu')
  } catch {
