...
// ─────────────────────────────────────────────────────────────────────────
// NETWORK FREE (ancora de crescimento) — 14/07/2026.
// `true` = Network aberto pra TODO admin logado, INCLUSIVE trial/free/vencido.
// Estrategia: a comunidade e' gratis e vira a ancora do site — a pessoa entra
// pelo Network, cria vinculo, e assina pra ter o painel completo.
// "Membro" (diretorio de @mencao, audiencia de @todos/avisos) continua sendo
// so quem JA ENTROU no Network (tem network_profiles) — nao os 1200+ admins.
// Pra voltar ao modo "so PRO", basta setar `false` (NETWORK_GA volta a valer).
// ─────────────────────────────────────────────────────────────────────────
export const NETWORK_FREE_FOR_ALL = true
...
// Quem pode usar "@todos" (marca todos os membros -> push pra todo mundo).
// So estas contas veem a opcao e conseguem disparar (validado no servidor).
export const MENTION_ALL_EMAILS = new Set(['leofritz180@gmail.com', 'darkzinmg7@gmail.com'])
export function canMentionAll(email) {
  return !!email && MENTION_ALL_EMAILS.has(String(email).toLowerCase())
}
