...
import { networkEnabled, NETWORK_EMAILS, NETWORK_GA, FOUNDERS_LIMIT, socialBetaEnabled } from './network-access'
...
export async function authNetwork(req) {
  ...
  const socialBeta = socialBetaEnabled(email)  // nova experiencia social (so contas de teste)
  ...
  let allowed = networkEnabled(email) || socialBeta
  ...
  return { user, profile, sb, isOwner, email, socialBeta }
}
