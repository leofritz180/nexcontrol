...
import { networkEnabled, NETWORK_EMAILS, NETWORK_GA, NETWORK_FREE_FOR_ALL, FOUNDERS_LIMIT, socialBetaEnabled, NETWORK_SOCIAL_GA, canMentionAll } from './network-access'
import { isApexLocked } from './rank-system'
...
// Verdadeiro se o tenant tem assinatura PRO ativa (usado no rollout geral e
// nos beneficios exclusivos de assinante dentro do Network — ex: editar perfil).
// Regra: linha ativa em `subscriptions` (expires_at nulo ou futuro) OU
// `tenants.subscription_status === 'active'`.
export async function hasActiveSubscription(sb, tenantId) {
...
export async function authNetwork(req) {
  ...
  // Acesso: allowlist (fase de teste) OU beta social OU Network FREE (todo admin,
  // inclusive trial/vencido — ancora de crescimento) OU GA com admin PRO ativo.
  let allowed = networkEnabled(email) || socialBetaAllow || NETWORK_FREE_FOR_ALL
  ...
  let myNp = null
  if (!isOwner) {
    const { data: np } = await sb.from('network_profiles').select('banned,can_mention_all').eq('user_id', user.id).maybeSingle()
    if (np?.banned === true) return { error: 'Você foi removido do Network.', status: 403 }
    myNp = np
  }

  // @todos: autorizado pela allowlist fixa OU pela flag por-usuario (owner libera no app).
  const mentionAllAllowed = isOwner || canMentionAll(email) || !!myNp?.can_mention_all

  // Rollout: no GA social, TODOS os autorizados recebem a versao rede social.
  const socialBeta = NETWORK_SOCIAL_GA || socialBetaAllow
  return { user, profile, sb, isOwner, email, socialBeta, canMentionAll: mentionAllAllowed }
}
...
// Membros do Network. Usado pra @mencao, diretorio e push de avisos/@todos.
// NETWORK FREE: membro = quem JA ENTROU no Network (tem network_profiles, criado
// pelo touchPresence no 1o acesso) e nao esta banido. Mantem a audiencia real e
// limitada — NAO os 1200+ admins do site. Fallback GA: admins PRO ativos.
export async function getMembers(sb) {
  const emails = [...NETWORK_EMAILS]
  let profs = []
  if (NETWORK_FREE_FOR_ALL) {
    // Paginado (cap 1000/req do Supabase) — a comunidade free pode passar disso.
    const allNps = []
    for (let from = 0; ; from += 1000) {
      const { data: page } = await sb.from('network_profiles')
        .select('user_id,banned').or('banned.is.null,banned.eq.false')
        .range(from, from + 999)
      allNps.push(...(page || []))
      if (!page || page.length < 1000) break
    }
    const memberIds = allNps.map(n => n.user_id)
    for (let i = 0; i < memberIds.length; i += 200) {
      const { data: admins } = await sb.from('profiles')
        .select('id,nome,email,tenant_id,role').eq('role', 'admin').in('id', memberIds.slice(i, i + 200))
      profs.push(...(admins || []))
    }
  } else if (NETWORK_GA) {
  ...
  for (let i = 0; i < ids.length; i += 200) {
    const { data: page } = await sb.from('network_profiles').select('user_id,avatar_url,instagram').in('user_id', ids.slice(i, i + 200))
    nps.push(...(page || []))
  }
...
// Selo FUNDADOR/VETERANO automatico pros primeiros a entrar (ate FOUNDERS_LIMIT).
// Com o Network FREE, so ASSINANTE PRO ganha o selo — free/trial nao consome
// vaga nem recebe (decisao do owner 14/07). Checagem de sub por ultimo: depois
// que as 100 vagas encherem, nem roda.
export async function maybeGrantFounder(sb, userId, alreadyFounder, tenantId) {
  try {
    ...
    if (!(await hasActiveSubscription(sb, tenantId))) return // free/trial: sem selo
...
export async function computePublicProfile(sb, userId, opts = {}) {
  ...
  const [opsRes, closedRes, netScore] = await Promise.all([
  ...
  const metasFechadas = fechadas.length
  const redeCount = {}
  let maiorMeta = 0
  ...
  // Depositantes processados = soma de quantidade_contas das metas FECHADAS de TODO o
  // tenant (todos operadores + admin). Mesma regra do dashboard/rank (countTenantDeposits).
  // Antes usava contas_remessa (métrica diferente) → o rank do admin não batia.
  ...
  const base = {
    ...
    founder,
    veterano: ageDays >= 60,
    canMentionAll: isOwner || canMentionAll((p.email || '').toLowerCase()) || !!np?.can_mention_all,
    apexLocked: isApexLocked((p.email || '').toLowerCase()), // owner sempre Apex no ranking
    bio: np?.bio || null,
