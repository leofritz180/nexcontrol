...
import { networkEnabled, NETWORK_EMAILS } from './network-access'
...
export async function getMembers(sb) {
  ...
  const [{ data: profs }, { data: nps }] = await Promise.all([
    ...
    sb.from('network_profiles').select('user_id,avatar_url'),
  ])
  const avById = {}; (nps || []).forEach(n => { avById[n.user_id] = n.avatar_url || null })
  return (profs || []).map(p => ({ id: p.id, name: publicName(p), avatar: avById[p.id] || null, color: colorFromId(p.id) }))
}
...
export async function buildAuthorMap(sb, authorIds) {
  ...
  const [{ data: profs }, { data: nps }] = await Promise.all([
    ...
    sb.from('network_profiles').select('user_id,verified,founder,rank,tag,avatar_url').in('user_id', ids),
  ])
