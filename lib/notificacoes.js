...
export const CATEGORIA = {
  'meta-criada': 'ciclo', 'meta-finalizada': 'ciclo', 'meta-fechada': 'ciclo', 'meta-fechada-operador': 'ciclo', 'conquista': 'ciclo',
...
const TEXTO = {
  'meta-criada': {
    ...
    engracado: d => ({ titulo: `${nome(d)} abriu o jogo 🎯`, corpo: `${d.contas || 0} DEP na ${REDE(d.rede)}. Começou a caçada.` }),
  ...
  'remessa-nova': {
    ...
    engracado: d => d.bonus
      ? ({ titulo: 'Bônus caiu na conta 🎁', corpo: `${nome(d)} registrou ${val(d.valor)} de bônus na ${REDE(d.rede)}.` })
      : Number(d.valor) >= 0
        ? ({ titulo: `${nome(d)} trouxe dinheiro 💸`, corpo: [val(d.valor), d.contas ? n(d.contas, 'conta', 'contas') : null, REDE(d.rede)].filter(Boolean).join(' · ') + prog(d) })
        : ({ titulo: `${nome(d)} tomou uma na ${REDE(d.rede)} 😬`, corpo: [val(d.valor), d.contas ? n(d.contas, 'conta', 'contas') : null].filter(Boolean).join(' · ') + prog(d) + '. Faz parte.' }),
    low: d => d.bonus
  ...
  'remessa-feedback': {
    ...
    engracado: d => {
      const base = `${val(d.valor)} (${rs(d.perConta)}/conta)`
      return {
        lucro:     { titulo: 'Aí sim 🔥', corpo: `${base}. Mantém esse ritmo.` },
        baixo:     { titulo: 'Tá de boa 👌', corpo: `${base} — prejuízo pequeno, o salário cobre.` },
        leve:      { titulo: 'Oscilou de leve 🌊', corpo: `${base}. Segue o baile.` },
        atencao:   { titulo: 'Fica de olho 👀', corpo: `${base}. Tá oscilando, não deixa virar onda.` },
        redobrada: { titulo: 'Hmm, tá pesando 🧐', corpo: `${base}. Que tal trocar o slot?` },
        ruim:      { titulo: 'Ih, deu ruim 😵', corpo: `${base}. Pausa, respira, revisa.` },
        negativo:  { titulo: 'Tá no vermelho 🚨', corpo: `${base}. Hora de mudar o caminho.` },
      }[d.nivel || nivelRemessa(d.valor, d.perConta)]
    },
    low: d => {
      const base = `${val(d.valor)} · ${rs(d.perConta)}/conta`
  ...
  'marco-meta': {
    ...
    engracado: d => d.marco >= 100
      ? (d.paraOperador ? { titulo: 'META BATIDA 🏆', corpo: `${d.feitas}/${d.alvo} na ${REDE(d.rede)}. Finaliza aí que o admin fecha.` } : { titulo: `${nome(d)} bateu a meta 🏆`, corpo: `${d.feitas}/${d.alvo} DEP na ${REDE(d.rede)}. Só falta finalizar.` })
      : (d.paraOperador ? { titulo: 'Metade do caminho 🏃', corpo: `${d.feitas}/${d.alvo} na ${REDE(d.rede)}. Agora é descida.` } : { titulo: `${nome(d)} tá na metade 🏃`, corpo: `${d.feitas}/${d.alvo} DEP na ${REDE(d.rede)}. Vai que vai.` }),
    low: d => d.marco >= 100
  ...
  'meta-finalizada': {
    ...
    engracado: d => ({ titulo: `${nome(d)} finalizou, bola na sua mão ⚽`, corpo: `${d.contas || 0} DEP ${REDE(d.rede)} · ${n(d.nRem || 0, 'remessa', 'remessas')} · ${val(d.valor)}. Fecha aí.` }),
  ...
  'meta-fechada': {
    ...
    engracado: d => Number(d.lucroFinal) >= 0
      ? ({ titulo: 'Fechou no lucro 🤑', corpo: `${d.contas || 0} DEP ${REDE(d.rede)} · ${val(d.lucroFinal)} no final.` })
      : ({ titulo: 'Fechou no vermelho 🩹', corpo: `${d.contas || 0} DEP ${REDE(d.rede)} · ${val(d.lucroFinal)}. Bora recuperar na próxima.` }),
  ...
  'meta-fechada-operador': {
    ...
    engracado: d => ({ titulo: 'Sua meta fechou ✅', corpo: `${d.contas || 0} DEP ${REDE(d.rede)} · suas remessas: ${val(d.valor)}. Próxima!` }),
  ...
  'alerta-operacao': {
    ...
    engracado: d => ({
      sequencia_negativa:   { titulo: 'Maré ruim ⚠️', corpo: `${n(d.streak || 0, 'remessa seguida', 'remessas seguidas')} no prejuízo (${rs(d.total)}). Troca a estratégia antes que vire tsunami.` },
      prejuizo_acima_media: { titulo: 'Essa doeu 😖', corpo: `${rs(d.perConta)}/conta, bem acima da sua média (${rs(d.media)}). Olho vivo.` },
      meta_parada:          { titulo: 'Meta tirando um cochilo 😴', corpo: `Sem remessa há ${d.horas || 0}h. Bora acordar ela?` },
      meta_sem_inicio:      { titulo: 'Meta parada na largada 🚦', corpo: `${d.contas || 0} DEP ${REDE(d.rede)} esperando a primeira remessa.` },
    }[d.insight] || { titulo: 'Alerta da operação', corpo: d.corpo || '' }),
    low: d => ({
  ...
  'network': {
    ...
    engracado: d => ({
      comentario: { titulo: `${nome(d)} comentou no seu post 💬`, corpo: d.trecho || '' },
      mencao:     { titulo: `${nome(d)} te chamou no Network 📣`, corpo: d.trecho || '' },
      todos:      { titulo: `${nome(d)} chamou geral 📣`, corpo: d.trecho || '' },
      aviso:      { titulo: 'Aviso novo no Network 📌', corpo: d.trecho || '' },
    }[d.evento] || { titulo: 'Network', corpo: d.trecho || '' }),
    low: d => ({
  ...
  'conquista': {
    ...
    engracado: d => d.evento === 'comissao'
      ? { titulo: 'Comissão na conta 💰', corpo: `${rs(d.valor)} via PIX. Aproveita.` }
      : { titulo: `Subiu de patente: ${d.patente || ''} 🎖️`, corpo: `${Number(d.depositantes || 0).toLocaleString('pt-BR')} depositantes processados. Respeita.` },
...
export function textoDe(tipo, p = {}, voz = VOZ_PADRAO) {
  ...
  try { return f(p) || t[VOZ_PADRAO](p) } catch { return t[VOZ_PADRAO](p) }
}
