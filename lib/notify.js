...
function generateMessage(event, data) {
  switch (event) {
    case 'meta_created': {
      ...
      return {
        title: 'NexControl',
        body: `Op. ${nome} iniciou meta - ${data.contas||0} DEP na ${(data.rede||'rede').toUpperCase()}`,
      }
    }
    case 'remessa_created': {
      ...
      return {
        title: 'NexControl',
        body: `Op. ${nome} finalizou 1 remessa - ${pos?'Lucro':'Prejuízo'}: R$ ${val}`,
      }
    }
    case 'meta_closed': {
      ...
      return {
        title: 'NexControl',
        body: `Meta encerrada - ${data.contas||0} DEP ${(data.rede||'').toUpperCase()} - ${pos?'Lucro':'Prejuízo'}: R$ ${val}`,
      }
    }
    default:
