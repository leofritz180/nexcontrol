...
// Pega so o primeiro nome (descarta sobrenomes) + capitaliza.
// Ex: 'thomas melo ferreira' → 'Thomas'
//     'JOAO Silva'          → 'Joao'
//     '@usuario'            → '@usuario' (sem espaco, mantem)
...
function generateMessage(event, data) {
  switch (event) {
    case 'meta_created': {
      const nome = firstName(data.operador)
      return {
        title: 'Nova meta!',
    ...
    case 'remessa_created': {
      ...
      if (data.tipo === 'bonus') {
        return {
          ...
          body: `${nome} finalizou remessa BONUS - +R$ ${val}`,
        }
      }
      return {
    ...
    case 'meta_closed': {
      ...
      return {
        ...
        body: `${data.contas||0} DEP ${(data.rede||'').toUpperCase()} finalizada — Resultado: ${pos?'+':'-'}R$ ${val}`,
      }
    }
    default:
...
export function notifyRemessaCreated(tenantId, operador, rede, resultado, tipo) {
  return dispatch(tenantId, 'remessa_created', { operador, rede, resultado, tipo })
}
