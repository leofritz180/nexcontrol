  ...
  switch (event) {
    case 'meta_created':
      return {
        ...
        body: `${(data.operador||'Operador').toUpperCase()} iniciou ${data.contas||0} DEP na ${(data.rede||'rede').toUpperCase()}`,
      }
    case 'remessa_created': {
      ...
      return {
        ...
        body: `${(data.operador||'Operador').toUpperCase()} registrou uma remessa — ${pos?'Lucro':'Prejuízo'}: R$ ${val}`,
      }
    }
    case 'meta_closed': {
      ...
      return {
        title: 'Meta fechada',
        body: `${data.contas||0} DEP ${(data.rede||'').toUpperCase()} encerrada — ${pos?'Lucro':'Prejuízo'} final: R$ ${val}`,
      }
    }
    default:
