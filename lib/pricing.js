...
// ═══════════════════════════════════════════════════════════════════════
// PACOTES FECHADOS — a oferta nova, DESLIGADA.
//
// Enquanto PACOTES_ATIVOS for false, NADA muda: calculatePrice devolve
// exatamente o que devolvia antes (base + por operador com desconto
// progressivo), e o checkout continua cobrando o mesmo. O codigo abaixo
// fica pronto e testado esperando a decisao.
//
// PRA LIGAR: trocar false por true e subir. Nao ha mais nada a fazer —
// o create-payment ja chama calculatePrice, entao o valor que vai pro
// Mercado Pago passa a ser o do pacote automaticamente. O MP nao sabe o
// que e plano: ele recebe um numero e devolve um QR.
//
...
// ═══════════════════════════════════════════════════════════════════════
export const PACOTES_ATIVOS = false
...
export function precoComPacote(operadores, { pro = false } = {}) {
  ...
  const porExcedente = Math.round(OP_BASE_PRICE * 0.75 * 100) / 100
  return {
    total: Math.round((topo.preco + excedente * porExcedente) * 100) / 100,
    pacote: topo,
    excedente,
  }
}
