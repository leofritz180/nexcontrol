...
export async function sendPushToAll(supabaseAdmin, pacote = {}) {
  const { title, body, tag } = pacote
  const { data: subs } = await supabaseAdmin
  ...
  const dead = []
  for (const sub of subs) {
    const porPapel = sub.profiles?.role === 'operator' ? '/operator' : '/admin'
    const result = await sendPush(sub, {
      ...pacote,
      title, body: body || '',
      url: pacote.url || porPapel,
      tag,
    })
    if (result.success) sent++
