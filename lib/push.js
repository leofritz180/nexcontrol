...
// Broadcast — envia para TODAS as inscrições. url decidido por papel.
export async function sendPushToAll(supabaseAdmin, { title, body, tag }) {
  const { data: subs } = await supabaseAdmin
    .from('push_subscriptions').select('id, endpoint, p256dh, auth, profiles(role)')
  if (!subs?.length) return { sent: 0, expired: 0, failed: 0, total: 0 }

  let sent = 0, expired = 0, failed = 0
  const dead = []
  for (const sub of subs) {
    const url = sub.profiles?.role === 'operator' ? '/operator' : '/admin'
    const result = await sendPush(sub, { title, body: body || '', url, tag })
    if (result.success) sent++
    else if (result.expired) { expired++; dead.push(sub.id) }
    else failed++
  }
  if (dead.length) await supabaseAdmin.from('push_subscriptions').delete().in('id', dead)
  return { sent, expired, failed, total: subs.length }
}
